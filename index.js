...
client.on('messageCreate', async (message) => {
	...
	const startedEmbed = new EmbedBuilder()
		.setColor('Random')
		.setTitle('<a:yukleniyor:1322203259556860047> Okuma işlemi başlatıldı..')
		.setAuthor({ name: message.author.tag, iconURL: message.author.displayAvatarURL() })
		.setDescription('Fotoğraf okunuyor, lütfen bekleyiniz.')
		.setTimestamp()
		.setFooter({text: `${client.user?.username} © 2025`, iconURL: client.user?.displayAvatarURL()})
	...
	if (!isSubscribed) {
		...
		const embed = new EmbedBuilder()
			...
			.setAuthor({ name: message.author.tag, iconURL: message.author.displayAvatarURL() })
			.setDescription(`<:ar_warn:1322203658128982096> **|** Kanala abone olmamışsın veya ben okuyamadım lütfen tam ekran görüntüsü atınız !`)
			.setTimestamp()
	...
	const embed = new EmbedBuilder()
		...
		.setTitle('Okuma işlemi sonlandırıldı')
		.setAuthor({ name: message.author.tag, iconURL: message.author.displayAvatarURL() })
		.setDescription(`<a:onay:1322203200278888512> **|** Tebrikler! Abone rolün verildi !`)
		.setTimestamp()
		.setFooter({ text: `${client.user?.username} © 2023`, iconURL: client.user?.displayAvatarURL() })
	msg.edit({embeds: [embed]});
});
